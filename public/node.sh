#!/bin/bash
# Node Installer Script for ZJ Panel
...
echo "======================================"
echo "    ZJ Panel Node Setup Script       "
echo "======================================"
...
mkdir -p /opt/zj-panel-node
cd /opt/zj-panel-node
...
cat << 'PKGEOF' > package.json
{
  "name": "zj-panel-node",
  "version": "1.0.0",
  "description": "Node agent for ZJ Panel",
  "main": "agent.js",
  "dependencies": {
    "express": "^4.18.2",
...
pm2 stop zj-node 2>/dev/null || true
pm2 delete zj-node 2>/dev/null || true
pm2 start agent.js --name zj-node
pm2 save
